    ...
    def get_batch(self, device):
        if self.subsample == 1:
            ...
            if self.subsample_counter < self.subsample - 1:
                ...
                batch_data = Batch(
                    full_batch_data.x[:, l:h, :].to(device),
...
class DistributedPriorDataLoader(PriorDataLoader):
    def __init__(
        self, load_path, n_gpus=1, n_chunks=2_000, store=False, subsample=1, partition=None
    ):
        self.path = load_path
        if store:
            self.partition = True  # new collections are partitioned
    ...
    def get_batch(self, device):
        ...
        if self.subsample == 1:
            print(
                f"get_batch {self.loaded_chunk_id} - {self.batch_counter}",
                file=sys.stderr,
                force=True,
            )
            _, batch_data = self.loaded_chunk[self.batch_counter]
            ...
            batch_data.target_y = batch_data.target_y.to(device)
            print(
                f"get_batch {self.loaded_chunk_id} - {self.batch_counter} - {batch_data.x.shape} - {batch_data.y.shape} - {batch_data.target_y.shape}",
                file=sys.stderr,
                force=True,
            )
            self.batch_counter += 1
            if self.batch_counter >= len(self.loaded_chunk):
                print(
                    f"load {(self.loaded_chunk_id + 1) % self.n_chunks}",
                    file=sys.stderr,
                    force=True,
                )
                self._load_chunk((self.loaded_chunk_id + 1) % self.n_chunks)
            ...
            print(
                f"get_batch {self.loaded_chunk_id} - {self.batch_counter} ({self.subsample_counter + 1}/{self.subsample})",
                file=sys.stderr,
                force=True,
            )
            _, full_batch_data = self.loaded_chunk[self.batch_counter]
...
def plot_prior(prior, samples=1000, buckets=50):
    s = np.array([prior() for _ in range(samples)])
    count, bins, ignored = plt.hist(s, buckets, density=True)
    print(s.min())
    plt.show()


trunc_norm_sampler_f = lambda mu, sigma: (
    lambda: stats.truncnorm((0 - mu) / sigma, (1000000 - mu) / sigma, loc=mu, scale=sigma).rvs(1)[0]
)
...
scaled_beta_sampler_f = lambda a, b, scale, minimum: (
