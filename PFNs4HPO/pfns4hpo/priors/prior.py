...
def safe_merge_batches_in_batch_dim(*batches, ignore_attributes=[]):
    ...
    e.g. multiple batch.x are concatenated in the batch dimension.
    ...
    assert all(f in merge_funcs for f in not_none_fields), (
        "Unknown fields encountered in `safe_merge_batches_in_batch_dim`."
    )
    return Batch(
        **{f: merge_funcs[f]([getattr(batch, f) for batch in batches]) for f in not_none_fields}
    )
...
def merge_batches(*batches, ignore_attributes=[]):
    ...
    def merge_attribute(attr_name, batch_sizes):
        ...
        if type(attr[0]) is torch.Tensor:
...
class PriorDataLoader(DataLoader, metaclass=ABCMeta):
    @abstractmethod
    def __init__(
        self, num_steps, batch_size, eval_pos_seq_len_sampler, seq_len_maximum, device, **kwargs
    ):
        """
