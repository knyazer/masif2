...
class TransformerModel(nn.Module):
    ...
    def init_weights(self):
        ...
        for layer in self.transformer_encoder.layers:
            ...
            attns = (
                layer.self_attn if isinstance(layer.self_attn, nn.ModuleList) else [layer.self_attn]
            )
            for attn in attns:
    ...
    def forward(self, *args, **kwargs):
        ...
        if len(args) == 3:
            ...
            assert all(
                kwarg in {"src_mask", "style", "only_return_standard_out"} for kwarg in kwargs
            ), (
...
class TransformerEncoderDiffInit(Module):
    ...
    def forward(
        self, src: Tensor, mask: Tensor | None = None, src_key_padding_mask: Tensor | None = None
    ) -> Tensor:
