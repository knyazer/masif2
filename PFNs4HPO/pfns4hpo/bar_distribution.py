...
class BarDistribution(nn.Module):
    def __init__(
        ...
        assert (self.bucket_widths >= 0.0).all(), (
    ...
    def icdf(self, logits, left_prob):
        ...
        idx = (
            torch.searchsorted(
                cumprobs, left_prob * torch.ones(*cumprobs.shape[:-1], 1, device=logits.device)
            )
            .squeeze(-1)
    ...
    def quantile(self, logits, center_prob=0.682):
        ...
        return torch.stack((self.icdf(logits, side_probs), self.icdf(logits, 1.0 - side_probs)), -1)
...
class FullSupportBarDistribution(BarDistribution):
    ...
    def repr(self, logits):
        ...
        bucket_means[0] = -side_normals[0].mean + self.borders[1]
        bucket_means[-1] = side_normals[1].mean + self.borders[-2]
        return (p, self.borders[:-1], self.borders[:-1] + self.bucket_widths)
...
def get_bucket_limits(
    num_outputs: int, full_range: tuple = None, ys: torch.Tensor = None, verbose: bool = False
):
