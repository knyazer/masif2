...
def _get_normalized_values(config, configuration_space: ConfigurationSpace) -> list[float]:
    ...
    neps_cfg = neps.search_spaces.search_space.SearchSpace(
        **{
            k: v
            for k, v in neps.search_spaces.search_space.pipeline_space_from_configspace(
                configuration_space
            ).items()
...
def prepare_data(benchmark="tabular-lcbench", task_id="adult", scenario={"train": {}, "test": {}}):
    ...
    benchmark = mfpbench.lcbench_tabular.LCBenchTabularBenchmark(
        task_id=task_id, datadir="/work/dlclarge1/mallik-lcpfn-hpo/pfns_hpo/data/lcbench-tabular"
    )
    ...
    train_data = torch.FloatTensor(
        sum(
            [
                [
                    [config_id, fidelity]
                    + _get_normalized_values(
                        config=benchmark.configs[str(config_id)], configuration_space=configspace
                    )
...
def compute_loglikelihood(
    model, benchmark="lcbench", task_id="adult", scenario={"train": {}, "test": {}}
):
    """Compute loglikelihood of a given scenario on a given benchmark."""

    x_train, y_train, x_test, y_test = prepare_data(
        benchmark=benchmark, task_id=task_id, scenario=scenario
    )
...
def get_partial_curves(benchmark="lcbench", n=10):
    ...
    print("loading benchmark...")
    benchmark = mfpbench.lcbench_tabular.LCBenchTabularBenchmark(
        task_id="adult", datadir="/work/dlclarge1/mallik-lcpfn-hpo/pfns_hpo/data/lcbench-tabular"
    )
...
def random_selection(
    benchmark="tabular-lcbench", task_id="adult", max_iteration=1000, return_schedule=False
):
    ...
    configurations = [
        _get_normalized_values(config=config, configuration_space=benchmark.space)
        for config in benchmark.configs.values()
    ]
    ...
    for _ in range(max_iteration):
        ...
        epochs_per_configuration[selected_configuration] = (
            epochs_per_configuration[selected_configuration] + 1
        )
        score = benchmark.query(
            config=benchmark.configs[str(selected_configuration)],
            at=epochs_per_configuration[selected_configuration].item(),
        ).score
...
def dynamic_selection(
    ...
    x_test = torch.FloatTensor([[0, 1] + config for config in configurations]).float()
    ...
    for _ in range(max_iteration):
        if criterion == "ucb":
            ...
            f_best = (
                torch.FloatTensor([max(trajectory) if len(trajectory) > 0 else 0])
                .expand(x_test.shape[0])
                .unsqueeze(-1)
            )
            utility = model.get_ei(
                x_train=x_train, y_train=y_train, x_test=x_test, f_best=f_best
            ).flatten()
        utility[
            epochs_per_configuration >= benchmark.end
        ] = -np.inf  # set to -inf the configurations that have reached the max budget
        ...
        score = benchmark.query(
            config=benchmark.configs[str(selected_configuration.item())],
            at=epochs_per_configuration[selected_configuration].item(),
        ).score

        x_train = torch.cat(
            [x_train, x_test[selected_configuration : selected_configuration + 1]], dim=0
        )
