...
class Benchmark:
    ...
    def query(self, dataset_name, tag, config_id, _int_id):
        """Query a run.
        ...
        try:
            return self.data[dataset_name][config_id]["log"][str(_int_id)][tag]
        except Exception as _:
            pass

        try:
            return self.data[dataset_name][config_id]["50"]["log"][str(_int_id)][tag]
        except Exception as _:
            pass

        try:
            return self.data[dataset_name][config_id]["50"]["log"][_int_id][tag]
        except Exception as _:
            pass
        try:
        ...
        raise ValueError(
            "Tag %s not found for config %s for dataset %s" % (tag, config_id, dataset_name)
        )
...
for name in bench.get_dataset_names():
    ...
    try:
        last_cnf = None
        for _id in range(bench.get_number_of_configs(name)):
            current_cnf = bench.get_config(name, _id)
            if last_cnf == current_cnf:
                continue
            for _int_id in range(1, 4):
                configs.append(
                    {
                        **current_cnf,
                        "data": np.array(
                            bench.query(name, tag, str(_id), _int_id), dtype=np.float16
                        ),
                    }
                )
